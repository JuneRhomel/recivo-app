...
export function Dropzone({ previewUrl, uploading, onFileSelected, onRemove }: DropzoneProps) {
  ...
  return (
    // The camera modal is a sibling of the drop area, not a child: nested, its
    // clicks would bubble into the drop area's onClick and open the file picker
    // on top of the modal.
    <>
      <div
        ...
        {previewUrl ? (
          ...
          <>
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-md border border-(--border) px-2.5 py-1 text-xs text-(--muted) hover:bg-black/4"
            >
              <Camera size={14} />
              Use camera
            </button>
      ...
      {cameraOpen && (
        <CameraCapture onClose={() => setCameraOpen(false)} onCapture={onFileSelected} />
      )}
    </>
  );
}
